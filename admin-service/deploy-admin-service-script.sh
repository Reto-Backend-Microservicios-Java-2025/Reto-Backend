...
# Script para el deployment del admin-service con Docker y Azure App Service
...
# Configuración
RESOURCE_GROUP="admin-service-rg"
LOCATION="eastus"
ACR_NAME="adminserviceacr$(date +%s)"
APP_PLAN="admin-service-plan"
WEB_APP="admin-service-app"

echo "🚀 Iniciando deployment del admin-service en Azure App Service..."
...
echo "Se crearán los siguientes recursos para admin-service:"
...
echo -e "${YELLOW}🔨 Compilando admin-service...${NC}"
mvn clean package -DskipTests

if [ ! -f "target/admin-service-0.0.1-SNAPSHOT.jar" ]; then
    echo -e "${RED}❌ Error: No se encontró el JAR compilado${NC}"
    exit 1
fi
echo -e "${GREEN}✅ Admin-service compilado exitosamente${NC}"
...
docker tag admin-service:latest $ACR_SERVER/admin-service:latest
docker push $ACR_SERVER/admin-service:latest
if [ $? -ne 0 ]; then
...
az webapp create \
  ...
  --plan $APP_PLAN \
  --name $WEB_APP \
  --deployment-container-image-name $ACR_SERVER/admin-service:latest
...
az webapp config container set \
  --name $WEB_APP \
  --resource-group $RESOURCE_GROUP \
  --docker-custom-image-name $ACR_SERVER/admin-service:latest \
...
echo ""
echo -e "${GREEN}🎉 ¡Deployment del admin-service completado exitosamente!${NC}"
echo -e "${GREEN}📋 Información del deployment:${NC}"
echo "   Resource Group: $RESOURCE_GROUP"
echo "   ACR Name: $ACR_NAME"
echo "   Web App Name: $WEB_APP"
echo "   URL: https://$APP_URL"
echo "   Spring Boot Admin Dashboard: https://$APP_URL"
echo ""
...
echo -e "${YELLOW}⏳ Esperando que el admin-service inicie...${NC}"
sleep 45
...
for attempt in {1..3}; do
    ...
    if [ "$HTTP_STATUS" = "200" ]; then
        echo -e "${GREEN}✅ ¡Admin-service funcionando correctamente!${NC}"
        echo -e "${GREEN}🎯 Spring Boot Admin Dashboard disponible en: https://$APP_URL${NC}"
        break
        ...
        if [ "$ROOT_STATUS" = "200" ]; then
            echo -e "${GREEN}✅ Admin-service funcionando (disponible en endpoint raíz)${NC}"
            echo -e "${GREEN}🎯 Spring Boot Admin Dashboard disponible en: https://$APP_URL${NC}"
            break
        else
...
if [ "$HTTP_STATUS" != "200" ] && [ "$ROOT_STATUS" != "200" ]; then
    echo -e "${YELLOW}⚠️  El admin-service aún se está iniciando${NC}"
    ...
    echo -e "${BLUE}🔍 Spring Boot Admin Server estará disponible para monitorear otros servicios${NC}"
fi